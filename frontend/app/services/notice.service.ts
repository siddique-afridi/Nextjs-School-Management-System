...
export async function fetchNotices(schoolId: string) {
  ...
  return asList(res.data).map((item) => mapNotice(item as Record<string, unknown>));
}

export async function createNotice(schoolId: string, data: { title: string; content: string }) {
